...
public sealed class EmployeeHandlersTests
{
    ...
    private static void SeedEmployees(IUnitOfWork<ApplicationDbContext> uow, Department dep)
    {
        ...
        repo.Insert(new ProcessBasedEmployee("Alice",  "E-001", dep) { Premium = new(10) });
        repo.Insert(new RateBasedEmployee   ("Bob",    "E-002", new(1000), dep, 5));
        repo.Insert(new Technologist       ("Charlie", "E-003", new(15),   dep));
    ...
    public async Task CreateEmployeeHandler_Should_Create_And_Return_Employee()
    {
        ...
        var model = new ProcessEmployeeCreateViewModel
        {
            Name        = "New Guy",
            InternalId  = "NG-007",
            DepartmentId= dep.Id,
        ...
        var saved = await uow.GetRepository<Employee>()
            .GetFirstOrDefaultAsync(predicate: e => e.InternalId == "NG-007", include: q => q.Include(x => x.Department));
