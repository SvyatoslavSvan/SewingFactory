...
public sealed class PointOfSaleHandlersTests
{
    ...
    private static async Task<UnitOfWork<ApplicationDbContext>> SeedAsync(IServiceProvider sp)
    {
        ...
        uow.DbContext.Set<WarehouseManagement.Domain.Entities.Inventory.PointOfSale>().Add(new WarehouseManagement.Domain.Entities.Inventory.PointOfSale("Main Store"));
    ...
    public async Task Create_PointOfSale_Returns_ReadViewModel()
    {
        ...
        var result = await handler.Handle(new CreateRequest<PointOfSaleCreateViewModel, WarehouseManagement.Domain.Entities.Inventory.PointOfSale, PointOfSaleDetailsReadViewModel>(vm, FakeUser), default);

        Assert.True(result.Ok);
        Assert.Equal("Outlet", result.Result!.Name);
        Assert.Equal(2, await uow.DbContext.Set<WarehouseManagement.Domain.Entities.Inventory.PointOfSale>().CountAsync());
    }
    ...
    public async Task GetAll_Returns_List()
    {
        ...
        var res = await handler.Handle(new GetAllRequest<WarehouseManagement.Domain.Entities.Inventory.PointOfSale, PointOfSaleReadViewModel>(FakeUser), default);
    ...
    public async Task Update_Changes_Name()
    {
        ...
        var entity = await uow.DbContext.Set<WarehouseManagement.Domain.Entities.Inventory.PointOfSale>()
            .AsNoTracking()
            .FirstAsync();
        ...
        var res = await handler.Handle(new UpdateRequest<PointOfSaleEditViewModel, WarehouseManagement.Domain.Entities.Inventory.PointOfSale>(editVm, FakeUser), default);
    ...
    public async Task Delete_Removes_Entity()
    {
        ...
        var id = await uow.DbContext.Set<WarehouseManagement.Domain.Entities.Inventory.PointOfSale>()
            .AsNoTracking()
        ...
        Assert.True(res.Ok);
        Assert.Empty(uow.DbContext.Set<WarehouseManagement.Domain.Entities.Inventory.PointOfSale>());
    }
    ...
    public async Task GetPointOfSaleById_Returns_Details_With_Stocks_And_Operations()
    {
        ...
        var cat = new WarehouseManagement.Domain.Entities.Garment.GarmentCategory("Tees", []);
        var model = new WarehouseManagement.Domain.Entities.Garment.GarmentModel("Classic", cat, Money.Zero);

        var pos = new WarehouseManagement.Domain.Entities.Inventory.PointOfSale("Main");
    ...
    public async Task InternalTransferRequest_Moves_Stock_Between_Points()
    {
        ...
        var sender = new WarehouseManagement.Domain.Entities.Inventory.PointOfSale("Sender");
        sender.AddStockItem(model);
        sender.StockItems.Single().IncreaseQuantity(8);

        var receiver = new WarehouseManagement.Domain.Entities.Inventory.PointOfSale("Receiver");
