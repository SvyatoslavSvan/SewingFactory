...
public sealed class AllOperationForStockReportProviderTests
{
    ...
    private static (WarehouseManagement.Domain.Entities.Inventory.PointOfSale pos, WarehouseManagement.Domain.Entities.Inventory.PointOfSale receiver,
        WarehouseManagement.Domain.Entities.Garment.GarmentModel model, DateRange range,
        decimal expectedTotal, int expectedRows) Arrange()
    {
        var cat = new WarehouseManagement.Domain.Entities.Garment.GarmentCategory("Футболки", []);
        var model = new WarehouseManagement.Domain.Entities.Garment.GarmentModel("F-100", cat, Amount(250));

        var posSender = new WarehouseManagement.Domain.Entities.Inventory.PointOfSale("ТТ-1");
        var posReceiver = new WarehouseManagement.Domain.Entities.Inventory.PointOfSale("ТТ-2");
