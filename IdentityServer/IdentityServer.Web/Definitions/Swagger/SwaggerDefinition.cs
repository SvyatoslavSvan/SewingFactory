...
namespace IdentityServer.Web.Definitions.Swagger;
...
public class SwaggerDefinition : AppDefinition
{
    ...
    public override void ConfigureServices(WebApplicationBuilder builder)
    {
        ...
        {
            options.SwaggerDoc("v1", new OpenApiInfo { Title = AppData.ServiceName, Version = AppVersion, Description = AppData.ServiceDescription });

            options.ResolveConflictingActions(resolver: x => x.First());
            ...
            {
                {
                    new OpenApiSecurityScheme
                    {
                        Reference = new OpenApiReference { Type = ReferenceType.SecurityScheme, Id = "oauth2" }, In = ParameterLocation.Cookie, Type = SecuritySchemeType.OAuth2
                    },
                    new List<string>()
                }
            });
